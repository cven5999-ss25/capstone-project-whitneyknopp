...
The data set used in this analysis comes from the United Nations High Commission on Refugees (UNHCR). It documents access to WASH services in refugee camps through numerous indicators collected over 12 years. The UNHCR WASH data are available as a package in R with documentation in github (SITE). 
...
```{r}
#| label: tab-water-qual
#| tab-cap: Statistics for water quality tests meeting Sphere Standards for east African countries
...
Sanitation is the second component of WASH. The Sphere standard for sanitation access is 2.2, which requires a maximum of 15 persons per toilet in emergencies. This standard might be influenced by the number of refugees at a refugee camp. @fig-san-pop plots persons per toilet as a function of refugee population to identify sanitation access trends between countries and refugee population. Little correlation between population and sanitation access is displayed @fig-san-pop, notably, in Uganda, Kenya and Tanzania, toilets are very frequently shared between less than 15 people in the majority of refugee camps.
...
Water and sanitation services are often related, as agencies tasked with WASH provision in emergencies may support both. @fig-wash-access plots water and sanitation access, as L/person/day and persons/toilet, respectively, for ongoing and previous emergencies in east Africa. Ideally, emergencies fall in the bottom right section created by the 2 horizontal lines, signifying that refugees have adequate drinking water access and less than the maximum number of persons per toilet. @tab-wash-access presents a summary of WASH access. 

```{r}
 #| label: fig-wash-access
 #| fig-cap: Sanitation access and metrics for refugees in east Africa

# compare drinking water and sanitation access
unhcrWASH_eastAfrica_tidy |> 
  ggplot() +
  geom_point(mapping = aes(y = liters_per_person_per_day,
                       x = persons_per_toilet,
                       color = country,
                       shape = post_emergency)) +
  geom_hline(yintercept = 15,
             color = "purple") +
...
```{r}
#| label: tab-wash-access
#| tab-cap: Summary of average water and santiation access during emergencies for east Africa
